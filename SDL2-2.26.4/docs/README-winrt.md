...
* What works:
  ...
  * compile-time platform detection for SDL programs.  The C/C++ #define,
    `__WINRegisterType__`, will be set to 1 (by SDL) when compiling for WinRT.
...
* What partially works:
  ...
  * SDLmain.  WinRT uses a different signature for each app's main() function.
    SDL-based apps that use this port must compile in SDL_winRegisterType_main_NonXAML.cpp
...
2. navigate to the directory containing SDL's source code, then into its
   ...
   - `SDL_winRegisterType_main_NonXAML.cpp`
   - `SDL2-WinRTResources.rc`
   - `SDL2-WinRTResource_BlankCursor.cur`
3. right-click on the file `SDL_winRegisterType_main_NonXAML.cpp` (as listed in your
...
#### Build fails with message, "error LNK2038: mismatch detected for 'vccorlib_lib_should_be_specified_before_msvcRegisterType_lib_to_linker'"
