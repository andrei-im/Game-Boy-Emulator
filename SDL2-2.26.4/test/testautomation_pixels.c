...
pixels_allocFreeFormat(void *arg)
{
  ...
  format = 0;
  SDLTest_Log("RGB Format: %s (%" SDL_PRIuint32_t ")", unknownFormat, format);
  ...
  if (result != NULL) {
    SDLTest_AssertCheck(result->format == format, "Verify value of result.format; expected: %" SDL_PRIuint32_t ", got %" SDL_PRIuint32_t, format, result->format);
    ...
    SDLTest_AssertCheck(masks == 0, "Verify value of result.[RGBA]mask combined; expected: 0, got %" SDL_PRIuint32_t, masks);
  ...
  for (i = 0; i < _numRGBPixelFormats; i++) {
    ...
    SDLTest_Log("RGB Format: %s (%" SDL_PRIuint32_t ")", _RGBPixelFormatsVerbose[i], format);
  ...
  for (i = 0; i < _numNonRGBPixelFormats; i++) {
    ...
    SDLTest_Log("non-RGB Format: %s (%" SDL_PRIuint32_t ")", _nonRGBPixelFormatsVerbose[i], format);
  ...
  for (i = 0; i < _numInvalidPixelFormats; i++) {
    ...
    result = SDL_AllocFormat(format);
    SDLTest_AssertPass("Call to SDL_AllocFormat(%" SDL_PRIuint32_t ")", format);
    SDLTest_AssertCheck(result == NULL, "Verify result is NULL");
    error = SDL_GetError();
...
pixels_getPixelFormatName(void *arg)
{
  ...
  for (i = 0; i < _numInvalidPixelFormats; i++) {
    format = _invalidPixelFormats[i];
    result = SDL_GetPixelFormatName(format);
    SDLTest_AssertPass("Call to SDL_GetPixelFormatName(%" SDL_PRIuint32_t ")", format);
    SDLTest_AssertCheck(result != NULL, "Verify result is not NULL");
    if (result != NULL) {
