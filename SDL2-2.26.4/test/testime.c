...
static int unifont_init(const char *fontname)
{
    ...
    SDL_RWclose(hexFile);
    SDL_Log("unifont: Loaded %" SDL_PRIuint32_t " glyphs.\n", numGlyphs);
    return 0;
}
...
static int unifont_load_texture(Uint32 textureID)
{
    ...
    {
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "unifont: Tried to load out of range texture %" SDL_PRIuint32_t "\n", textureID);
        return -1;
    }
    ...
    for (i = 0; i < state->num_windows; ++i)
    {
        ...
        if (tex == NULL)
        {
            SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "unifont: Failed to create texture %" SDL_PRIuint32_t " for renderer %d.\n", textureID, i);
            return -1;
        }
        ...
        {
            SDL_Log("unifont error: Failed to update texture %" SDL_PRIuint32_t " data for renderer %d.\n", textureID, i);
        }
    }
