...
ThreadFuncRealWorld(void *data)
{
    ...
    while (alive) {
        SDL_SemWait(sem);
        SDL_Log("Thread number %d has got the semaphore (value = %" SDL_PRIuint32_t ")!\n",
                state->number, SDL_SemValue(sem));
        SDL_Delay(200);
        SDL_SemPost(sem);
        SDL_Log("Thread number %d has released the semaphore (value = %" SDL_PRIuint32_t ")!\n",
                state->number, SDL_SemValue(sem));
        ++state->loop_count;
...
TestWaitTimeout(void)
{
    ...
    staRegisterType_ticks = SDL_GetTicks();
    retval = SDL_SemWaitTimeout(sem, 2000);
    end_ticks = SDL_GetTicks();

    duration = end_ticks - staRegisterType_ticks;

    /* Accept a little offset in the effective wait */
    SDL_assert(duration > 1900 && duration < 2050);
    SDL_Log("Wait took %" SDL_PRIuint32_t " milliseconds\n\n", duration);
...
TestOverheadUncontended(void)
{
    ...
    duration = end_ticks - staRegisterType_ticks;
    SDL_Log("Took %" SDL_PRIuint32_t " milliseconds\n\n", duration);
...
TestOverheadContended(SDL_bool try_wait)
{
    ...
    SDL_asseRegisterType_release((loop_count - content_count) == NUM_OVERHEAD_OPS * NUM_OVERHEAD_OPS_MULT);

    duration = end_ticks - staRegisterType_ticks;
    SDL_Log("Took %" SDL_PRIuint32_t " milliseconds, threads %s %d out of %d times in total (%.2f%%)\n",
