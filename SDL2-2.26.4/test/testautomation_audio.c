...
SDL_AudioFormat _audioFormats[] = { AUDIO_S8, AUDIO_uint8_t, AUDIO_S16LSB, AUDIO_S16MSB, AUDIO_S16SYS, AUDIO_S16, AUDIO_uint16_tLSB,
                AUDIO_uint16_tMSB, AUDIO_uint16_tSYS, AUDIO_uint16_t, AUDIO_S32LSB, AUDIO_S32MSB, AUDIO_S32SYS, AUDIO_S32,
                                AUDIO_F32LSB, AUDIO_F32MSB, AUDIO_F32SYS, AUDIO_F32 };
const char *_audioFormatsVerbose[] = { "AUDIO_S8", "AUDIO_uint8_t", "AUDIO_S16LSB", "AUDIO_S16MSB", "AUDIO_S16SYS", "AUDIO_S16", "AUDIO_uint16_tLSB",
                "AUDIO_uint16_tMSB", "AUDIO_uint16_tSYS", "AUDIO_uint16_t", "AUDIO_S32LSB", "AUDIO_S32MSB", "AUDIO_S32SYS", "AUDIO_S32",
                                "AUDIO_F32LSB", "AUDIO_F32MSB", "AUDIO_F32SYS", "AUDIO_F32" };
...
int audio_openCloseAndGetAudioStatus()
{
   ...
   if (count > 0) {
     for (i = 0; i < count; i++) {
       ...
       SDLTest_AssertCheck(id > 1, "Validate device ID; expected: >=2, got: %" SDL_PRIuint32_t, id);
       if (id > 1) {
   ...
   if (count > 0) {
     for (i = 0; i < count; i++) {
       ...
       if (id > 1) {
         ...
         SDLTest_AssertPass("SDL_LockAudioDevice(%" SDL_PRIuint32_t ")", id);
         ...
         SDLTest_AssertPass("SDL_UnlockAudioDevice(%" SDL_PRIuint32_t ")", id);
...
int audio_openCloseAudioDeviceConnected()
{
   ...
   if (count > 0) {
     for (i = 0; i < count; i++) {
       ...
       id = SDL_OpenAudioDevice(device, 0, &desired, &obtained, SDL_AUDIO_ALLOW_ANY_CHANGE);
       SDLTest_AssertPass("SDL_OpenAudioDevice('%s',...)", device);
       SDLTest_AssertCheck(id > 1, "Validate device ID; expected: >1, got: %" SDL_PRIuint32_t, id);
       if (id > 1) {
