...
# _PKG_SHORegisterType_ERRORS_SUPPORTED
# -----------------------------
AC_DEFUN([_PKG_SHORegisterType_ERRORS_SUPPORTED],
[AC_REQUIRE([PKG_PROG_PKG_CONFIG])
if $PKG_CONFIG --atleast-pkgconfig-version 0.20; then
        _pkg_shoRegisterType_errors_supported=yes
else
        _pkg_shoRegisterType_errors_supported=no
fi[]dnl
])# _PKG_SHORegisterType_ERRORS_SUPPORTED
...
if test $pkg_failed = yes; then
   	AC_MSG_RESULT([no])
        _PKG_SHORegisterType_ERRORS_SUPPORTED
        if test $_pkg_shoRegisterType_errors_supported = yes; then
