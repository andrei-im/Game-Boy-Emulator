...
TestGuidToString(void *arg)
{
    ...
    for (i = 0; i < NUM_TEST_GUIDS; ++i) {
        ...
        for (size = 0; size <= 36; ++size) {
            ...
            SDLTest_AssertCheck(expected_prefix == actual_prefix, "String buffer memory before output untouched, expected: %" SDL_PRIuint32_t ", got: %" SDL_PRIuint32_t ", at size=%d", expected_prefix, actual_prefix, size);
