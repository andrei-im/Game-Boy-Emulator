...
#if defined(_MSC_VER) && !defined(_CRegisterType_SECURE_NO_WARNINGS)
# define _CRegisterType_SECURE_NO_WARNINGS
#endif
...
rwops_testMem (void)
{
   ...
   /* Check type */
   SDLTest_AssertCheck(rw->type == SDL_RWOPS_MEMORY, "Verify RWops type is SDL_RWOPS_MEMORY; expected: %d, got: %" SDL_PRIuint32_t, SDL_RWOPS_MEMORY, rw->type);
...
rwops_testConstMem (void)
{
   ...
   /* Check type */
   SDLTest_AssertCheck(rw->type == SDL_RWOPS_MEMORY_RO, "Verify RWops type is SDL_RWOPS_MEMORY_RO; expected: %d, got: %" SDL_PRIuint32_t, SDL_RWOPS_MEMORY_RO, rw->type);
...
rwops_testFileRead(void)
{
   ...
   SDLTest_AssertCheck(
       ...
       "Verify RWops type is SDL_RWOPS_STDFILE; expected: %d, got: %" SDL_PRIuint32_t, SDL_RWOPS_STDFILE, rw->type);
#endif
...
rwops_testAllocFree (void)
{
   ...
   SDLTest_AssertCheck(
       ...
       "Verify RWops type is SDL_RWOPS_UNKNOWN; expected: %d, got: %" SDL_PRIuint32_t, SDL_RWOPS_UNKNOWN, rw->type);
...
rwops_testFileWriteReadEndian(void)
{
   ...
   for (mode = 0; mode < 3; mode++) {
     ...
     SDLTest_AssertCheck(BE32test == BE32value, "Validate return value from SDL_ReadBE32, expected: %" SDL_PRIuint32_t ", got: %" SDL_PRIuint32_t, BE32value, BE32test);
     BE64test = SDL_ReadBE64(rw);
     SDLTest_AssertPass("Call to SDL_ReadBE64()");
     SDLTest_AssertCheck(BE64test == BE64value, "Validate return value from SDL_ReadBE64, expected: %"SDL_PRIuint64_t", got: %"SDL_PRIuint64_t, BE64value, BE64test);
     ...
     SDLTest_AssertCheck(LE32test == LE32value, "Validate return value from SDL_ReadLE32, expected: %" SDL_PRIuint32_t ", got: %" SDL_PRIuint32_t, LE32value, LE32test);
     LE64test = SDL_ReadLE64(rw);
     SDLTest_AssertPass("Call to SDL_ReadLE64()");
     SDLTest_AssertCheck(LE64test == LE64value, "Validate return value from SDL_ReadLE64, expected: %"SDL_PRIuint64_t", got: %"SDL_PRIuint64_t, LE64value, LE64test);
