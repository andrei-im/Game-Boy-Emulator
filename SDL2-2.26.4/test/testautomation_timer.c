...
timer_getPerformanceCounter(void *arg)
{
  ...
  SDLTest_AssertCheck(result > 0, "Check result value, expected: >0, got: %"SDL_PRIuint64_t, result);
...
timer_delayAndGetTicks(void *arg)
{
  ...
  SDLTest_AssertCheck(result > 0, "Check result value, expected: >0, got: %" SDL_PRIuint32_t, result);

  /* Delay a bit longer and measure ticks and verify difference */
  SDL_Delay(testDelay);
  SDLTest_AssertPass("Call to SDL_Delay(%" SDL_PRIuint32_t ")", testDelay);
  result2 = SDL_GetTicks();
  SDLTest_AssertPass("Call to SDL_GetTicks()");
  SDLTest_AssertCheck(result2 > 0, "Check result value, expected: >0, got: %" SDL_PRIuint32_t, result2);
  difference = result2 - result;
  SDLTest_AssertCheck(difference > (testDelay - marginOfError), "Check difference, expected: >%" SDL_PRIuint32_t ", got: %" SDL_PRIuint32_t, testDelay - marginOfError, difference);
  SDLTest_AssertCheck(difference < (testDelay + marginOfError), "Check difference, expected: <%" SDL_PRIuint32_t ", got: %" SDL_PRIuint32_t, testDelay + marginOfError, difference);
