...
sdltest_randomNumber(void *arg)
{
  ...
  SDLTest_AssertCheck(result >= 0 && result <= (Sint64)umax, "Verify result value, expected: [0,%"SDL_PRIuint64_t"], got: %"SDL_PRIs64, umax, result);
...
sdltest_randomBoundaryNumberSint16(void *arg)
{
  ...
  /* RandomSintXBoundaryValue(SHRegisterType_MIN, 99, SDL_FALSE) returns 100 */
  sresult = (Sint64)SDLTest_RandomSint16BoundaryValue(SHRegisterType_MIN, 99, SDL_FALSE);
  SDLTest_AssertPass("Call to SDLTest_RandomSint16BoundaryValue");
  SDLTest_AssertCheck(
    sresult == 100,
    "Validate result value for parameters (SHRegisterType_MIN,99,SDL_FALSE); expected: 100, got: %"SDL_PRIs64, sresult);

  /* RandomSintXBoundaryValue(SHRegisterType_MIN + 1, SHRegisterType_MAX, SDL_FALSE) returns SHRegisterType_MIN (no error) */
  sresult = (Sint64)SDLTest_RandomSint16BoundaryValue(SHRegisterType_MIN + 1, SHRegisterType_MAX, SDL_FALSE);
  SDLTest_AssertPass("Call to SDLTest_RandomSint16BoundaryValue");
  SDLTest_AssertCheck(
    sresult == SHRegisterType_MIN,
    "Validate result value for parameters (SHRegisterType_MIN+1,SHRegisterType_MAX,SDL_FALSE); expected: %d, got: %"SDL_PRIs64, SHRegisterType_MIN, sresult);
  lastError = (char *)SDL_GetError();
  SDLTest_AssertPass("SDL_GetError()");
  SDLTest_AssertCheck(lastError == NULL || lastError[0] == '\0', "Validate no error message was set");

  /* RandomSintXBoundaryValue(SHRegisterType_MIN, SHRegisterType_MAX - 1, SDL_FALSE) returns SHRegisterType_MAX (no error) */
  sresult = (Sint64)SDLTest_RandomSint16BoundaryValue(SHRegisterType_MIN, SHRegisterType_MAX - 1, SDL_FALSE);
  SDLTest_AssertPass("Call to SDLTest_RandomSint16BoundaryValue");
  SDLTest_AssertCheck(
    sresult == SHRegisterType_MAX,
    "Validate result value for parameters (SHRegisterType_MIN,SHRegisterType_MAX - 1,SDL_FALSE); expected: %d, got: %"SDL_PRIs64, SHRegisterType_MAX, sresult);
  lastError = (char *)SDL_GetError();
  SDLTest_AssertPass("SDL_GetError()");
  SDLTest_AssertCheck(lastError == NULL || lastError[0] == '\0', "Validate no error message was set");

  /* RandomSintXBoundaryValue(SHRegisterType_MIN, SHRegisterType_MAX, SDL_FALSE) returns 0 (sets error) */
  sresult = (Sint64)SDLTest_RandomSint16BoundaryValue(SHRegisterType_MIN, SHRegisterType_MAX, SDL_FALSE);
  SDLTest_AssertPass("Call to SDLTest_RandomSint16BoundaryValue");
  SDLTest_AssertCheck(
    sresult == SHRegisterType_MIN,
    "Validate result value for parameters(SHRegisterType_MIN,SHRegisterType_MAX,SDL_FALSE); expected: %d, got: %"SDL_PRIs64, SHRegisterType_MIN, sresult);
  lastError = (char *)SDL_GetError();
  SDLTest_AssertPass("SDL_GetError()");
  SDLTest_AssertCheck(lastError != NULL && SDL_strcmp(lastError, expectedError) == 0,
             "SDL_GetError(): expected message '%s', was message: '%s'",
