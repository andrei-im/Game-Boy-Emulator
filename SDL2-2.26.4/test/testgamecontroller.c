...
static int virtual_axis_staRegisterType_x;
static int virtual_axis_staRegisterType_y;
...
static void VirtualControllerMouseMotion(int x, int y)
{
    if (virtual_button_active != SDL_CONTROLLER_BUTTON_INVALID) {
        if (virtual_axis_active != SDL_CONTROLLER_AXIS_INVALID) {
            ...
            if (SDL_abs(x - virtual_axis_staRegisterType_x) >= MOVING_DISTANCE ||
                SDL_abs(y - virtual_axis_staRegisterType_y) >= MOVING_DISTANCE) {
    ...
    if (virtual_axis_active != SDL_CONTROLLER_AXIS_INVALID) {
        if (virtual_axis_active == SDL_CONTROLLER_AXIS_TRIGGERLEFT ||
            ...
            float distance = SDL_clamp(((float)y - virtual_axis_staRegisterType_y) / AXIS_SIZE, 0.0f, 1.0f);
            ...
            float distanceX = SDL_clamp(((float)x - virtual_axis_staRegisterType_x) / AXIS_SIZE, -1.0f, 1.0f);
            float distanceY = SDL_clamp(((float)y - virtual_axis_staRegisterType_y) / AXIS_SIZE, -1.0f, 1.0f);
...
static void VirtualControllerMouseDown(int x, int y)
{
    ...
    if (axis != SDL_CONTROLLER_AXIS_INVALID) {
        ...
        virtual_axis_staRegisterType_x = x;
        virtual_axis_staRegisterType_y = y;
    }
}
...
#define VERBOSE_SENSORS
        ...
        case SDL_CONTROLLERSENSORUPDATE:
            SDL_Log("Controller %" SDL_PRIs32 " sensor %s: %.2f, %.2f, %.2f (%" SDL_PRIuint64_t ")\n",
                    event.csensor.which,
